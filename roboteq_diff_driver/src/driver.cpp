...
#define MOTOR_REDUCTION_RATIO 4.33333333
#define CNT_PER_ROUND_ROTOR 60
...
public:
  ...
  void odom_publish();
#ifdef _ODOM_COVAR_SERVER
  void odom_covar_callback(
      const roboteq_diff_msgs::RequestOdometryCovariancesRequest &req,
      roboteq_diff_msgs::RequestOdometryCovariancesResponse &res);
#endif
...
MainNode::MainNode()
    : starttime(0), hstimer(0), mstimer(0), odom_idx(0), odom_encoder_toss(5),
      odom_encoder_left(0), odom_encoder_right(0), odom_x(0.0), odom_y(0.0),
      odom_yaw(0.0), odom_last_x(0.0), odom_last_y(0.0), odom_last_yaw(0.0),
      odom_last_time(0),
#ifdef _ODOM_SENSORS
      voltage(0.0), current_right(0.0), current_left(0.0), energy(0.0),
      temperature(0.0), current_last_time(0),
#endif
      pub_odom_tf(true), open_loop(false), baud(115200), wheel_circumference(0),
      track_width(0), encoder_ppr(0), encoder_cpr(0)
{
  ...
  nhLocal.param("wheel_circumference", wheel_circumference, 0.354);
  ROS_INFO_STREAM("wheel_circumference: " << wheel_circumference);
  nhLocal.param("track_width", track_width, 0.23); // 23cm signage
...
void MainNode::cmdvel_callback(const geometry_msgs::Twist &twist_msg)
{
  ...
  float left_speed =
      twist_msg.linear.x - track_width * twist_msg.angular.z / 2.0;
#ifdef _CMDVEL_DEBUG
  ROS_DEBUG_STREAM("cmdvel speed right: " << right_speed
                                          << " left: " << left_speed);
#endif
  ...
  if (open_loop)
  {
    ...
    int32_t right_power = right_speed / wheel_circumference * 60.0 / 82.0 *
                          1000.0 * MOTOR_REDUCTION_RATIO;
    int32_t left_power = left_speed / wheel_circumference * 60.0 / 82.0 *
                         1000.0 * MOTOR_REDUCTION_RATIO;
#ifdef _CMDVEL_DEBUG
    ROS_DEBUG_STREAM("cmdvel power right: " << right_power
                                            << " left: " << left_power);
#endif
  ...
  else
  {
    // motor speed (rpm)
    int32_t right_rpm =
        right_speed / wheel_circumference * 60.0 * MOTOR_REDUCTION_RATIO;
    int32_t left_rpm =
        left_speed / wheel_circumference * 60.0 * MOTOR_REDUCTION_RATIO;
#ifdef _CMDVEL_DEBUG
    ROS_DEBUG_STREAM("cmdvel rpm right: " << right_rpm
                                          << " left: " << left_rpm);
#endif
...
void MainNode::cmdvel_setup()
{
  ...
  if (open_loop)
  {
    ...
    controller.write("^KPF 1 200000\r"); // FOCのPゲイン
    controller.write("^KPF 2 0\r");
    controller.write("^KPF 3 200000\r");
    controller.write("^KPF 4 0\r");

    controller.write("^KIF 1 5000000\r"); // FOCのIゲイン
    controller.write("^KIF 2 0\r");
    controller.write("^KIF 3 5000000\r");
    controller.write("^KIF 4 0\r");
  }
  else
  {
    ...
    controller.write("^KPF 1 0\r"); // FOCのPゲイン
    controller.write("^KPF 2 0\r");
    controller.write("^KPF 3 0\r");
    controller.write("^KPF 4 0\r");

    controller.write("^KIF 1 0\r"); // FOCのIゲイン
    controller.write("^KIF 2 0\r");
    controller.write("^KIF 3 0\r");
    controller.write("^KIF 4 0\r");

    // set PID parameters (gain * 10)
    controller.write("^KP 1 500000\r");
    controller.write("^KP 2 500000\r");
    controller.write("^KI 1 10000000\r");
    controller.write("^KI 2 10000000\r");
  ...
  controller.write("^MXRPM 1 1000\r");
  controller.write("^MXRPM 2 1000\r");

  // set max acceleration rate (5000 rpm/s * 10)
  controller.write("^MAC 1 50000\r");
  controller.write("^MAC 2 50000\r");

  // set max deceleration rate (5000 rpm/s * 10)
  controller.write("^MDEC 1 50000\r");
  controller.write("^MDEC 2 50000\r");

  // set encoder mode (18 for feedback on motor1, 34 for feedback on motor2)
  controller.write("^EMOD 1 0\r");
  controller.write("^EMOD 2 0\r");

  // added for HBL2360
  controller.write("^PWMF 250\r");   // 250kHz
  controller.write("^BPOL 10 10\r"); // hall sensor のpole数
  controller.write("^BLSTD 3 3\r");  // stal detecion
  controller.write("^BMOD 1 1\r");   // blushlessのモード。1でFOC 0で短形波
  controller.write("^BFBK 1 1\r");   // Brushless feedback sesnor 1: Hall
  ...
  cmdvel_sub =
      nh.subscribe(cmdvel_topic, 1000, &MainNode::cmdvel_callback, this);
}
...
#ifdef _ODOM_COVAR_SERVER
void MainNode::odom_covar_callback(
    const roboteq_diff_msgs::RequestOdometryCovariancesRequest &req,
    roboteq_diff_msgs::RequestOdometryCovariancesResponse &res)
{
...
(1e3) ;
  ...
  position.twist.covariance =  boost::assign::list_of(1e-3) (0)   (0)  (0)  (0)
(0)
                                                      (0) (1e-3)  (0)  (0)  (0)
(0)
                                                      (0)   (0)  (1e6) (0)  (0)
...
To estimate velocity covariance you should know TICKSMM (128) and SIPCYCLE (100)
parameters of your robot (written in your robots flash memory and not accessible
with Aria). First parameter tells you how many encoder impulses (count) gets
generated by your robot's forward movement of 1 mm. Second parameter tells you
number of milliseconds between two consecutive Server Information Packets from
your robot. The values in the parentheses are for P3-DX (ARCOS).

So an error in determining velocity could come from missing an encoder impulse
in a cycle. This would result in 1/TICKSMM/SIPCYCLE velocity error (mm/ms or
m/s) for one wheel. For P3-DX parameters above, this value is 7.8125e-05. Note
that you would err by the same absolute amount of velocity in the next cycle.
Gearbox also plays a role in velocity error, but you would need to measure to
find the exact amount. As a rule of thumb, I would at least double the previous
amount in order to include gearbox error.

Now that we have determined maximum error of a single wheel's (transversal)
velocity, i.e. we expect 99.7% of errors to be less than this number, we can
determine sigma = max_err/3 and C = sigma^2. Translational and rotational
velocities are determined from left and right wheel velocities like this:

v = (v_R + v_L)/2

w = (v_R - v_L)/(2d)

So the covariance for transversal velocity would be (1/2)^2 2C and the
covariance for rotational velocity would be (1/(2d))^2 2C. The d parameter is
1/DiffConvFactor and is accessible from Aria (ArRobot::getDiffConvFactor()).
...
void MainNode::odom_setup()
{
  ...
  odom_covar_server = nh.advertiseService("roboteq/odom_covar_srv",
                                          &MainNode::odom_covar_callback, this);
#endif
  ...
  current_pub =
      nh.advertise<roboteq_diff_msgs::Duplex>("roboteq/current", 1000);
  ...
  temperature_pub =
      nh.advertise<std_msgs::Float32>("roboteq/temperature", 1000);
#endif
...
void MainNode::odom_stream()
{
  ...
  // ただし、100 にて　odomのpublish rateは 8.6Hz /  33にて odomのpublish rateは
  // 26Hz
  // start encoder output (30 hz)
  controller.write("# C_?BCR_# 20\r");
#endif
  controller.flush();
}
...
#define CNT_BCR_LENGTH 4

      // BCR= is encoder counts
      // if (odom_buf[0] == 'C' && odom_buf[1] == 'R' && odom_buf[2] == '=')
      if (odom_buf[0] == 'B' && odom_buf[1] == 'C' && odom_buf[2] == 'R')
      {
        int delim;
        for (delim = CNT_BCR_LENGTH; delim < odom_idx; delim++)
        {
          ...
          if (odom_buf[delim] == ':')
          {
            odom_buf[delim] = 0;
            odom_encoder_right =
                (int32_t)strtol(odom_buf + CNT_BCR_LENGTH, NULL, 10);
            odom_encoder_left = (int32_t)strtol(odom_buf + delim + 1, NULL, 10);
#ifdef _ODOM_DEBUG
// ROS_DEBUG_STREAM("encoder right: " << odom_encoder_right << " left: " <<
// odom_encoder_left);
#endif
            odom_publish();
            break;
      ...
      else if (odom_buf[0] == 'V' && odom_buf[1] == '=')
      {
        ...
        for (int delim = 2; delim <= odom_idx; delim++)
        {
          if (odom_buf[delim] == ':' || odom_buf[delim] == 0)
          {
            ...
            {
            case 0:
//              odom_internal_voltage = (float)strtol(odom_buf+start, NULL, 10)
/ 10.0;
              break;
            case 1:
...
#ifdef _ODOM_DEBUG
// ROS_DEBUG_STREAM("current right: " << current_right << " left: " <<
// current_left);
#endif
...
void MainNode::odom_publish()
{
  ...
  ROS_DEBUG_STREAM(odom_encoder_right);
  ROS_DEBUG_STREAM(" left: ");
  ROS_DEBUG_STREAM(odom_encoder_left);
  ROS_DEBUG_STREAM(" dt: ");
  ROS_DEBUG_STREAM(dt);
  ROS_DEBUG_STREAM("");
#endif
  ...
  // float linear = ((float)odom_encoder_right / (float)encoder_cpr *
  // wheel_circumference + (float)odom_encoder_left / (float)encoder_cpr *
  // wheel_circumference) / 2.0;
  float linear = ((float)odom_encoder_right * wheel_circumference +
                  (float)odom_encoder_left * wheel_circumference) /
                 (MOTOR_REDUCTION_RATIO * CNT_PER_ROUND_ROTOR) / 2.0;
  //  float angular = ((float)odom_encoder_right / (float)encoder_cpr *
  //  wheel_circumference - (float)odom_encoder_left / (float)encoder_cpr *
  //  wheel_circumference) / track_width * -1.0;
  float angular = ((float)odom_encoder_right * wheel_circumference -
                   (float)odom_encoder_left * wheel_circumference) /
                  (MOTOR_REDUCTION_RATIO * CNT_PER_ROUND_ROTOR) / track_width;
#ifdef _ODOM_DEBUG
  ROS_DEBUG_STREAM("linear: ");
  ROS_DEBUG_STREAM(linear);
  ROS_DEBUG_STREAM(" angular: ");
  ROS_DEBUG_STREAM(angular);
  ROS_DEBUG_STREAM("");
#endif
...
#ifdef _ODOM_DEBUG
  ROS_DEBUG_STREAM("odom x: " << odom_x << " y: " << odom_y
                              << " yaw: " << odom_yaw);
#endif
...
#ifdef _ODOM_DEBUG
  ROS_DEBUG_STREAM("velocity vx: " << odom_x << " vy: " << odom_y
                                   << " vyaw: " << odom_yaw);
#endif
  odom_last_x = odom_x;
  odom_last_y = odom_y;
...
int MainNode::run()
{
  ...
  while (ros::ok())
  {
    ...
    uint32_t nowtime = millis();
    // ROS_INFO_STREAM("loop nowtime: " << nowtime << " lstimer: " << lstimer <<
    // " delta: " << DELTAT(nowtime,lstimer) << " / " << (nowtime-lstimer));
    // uint32_t delta = DELTAT(nowtime,lstimer);
    // ROS_INFO_STREAM("loop nowtime: " << nowtime << " lstimer: " << lstimer <<
    // " delta: " << delta << " / " << (nowtime-lstimer));
